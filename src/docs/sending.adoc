...
An optional closure that can be used to "post process" the message after it has been converted into a message but before it has been sent. This closure acts as the implementation of the {post_process_message}[postProcessMessage()] method of the {message_post_processor}[MessagePostProcessor] class.
...
include::sending/postProcessing.adoc[]
include::sending/otherTemplates.adoc[]