package grails.plugins.jms.connection;
...
public class JmsFallbackConnectionFactory implements ConnectionFactory {
    ...
    public JmsFallbackConnectionFactory(ConnectionFactory parent) {
        ...
        if (log.isTraceEnabled())
            log.trace("Creating a " + getClass().getSimpleName() + " instance with primary [" + parent + "]");
        setParent(parent);
    }
    ...
    public void setParent(ConnectionFactory parent) {
        if (parent == null) {
            if (log.isWarnEnabled())
                log.warn("Assigning a null parent to a " + getClass().getSimpleName() + ": will always use local fallback JMS");
            parentRef.set(null);
        } else {
            if (log.isDebugEnabled())
                log.debug("Setting a non-null parent to a " + getClass().getSimpleName() + ": [" + parent + "]");
    ...
    public void setFallback(ConnectionFactory fallback) {
        if (fallback == null) {
            if (log.isWarnEnabled())
                log.warn("Assigning a null fallback to a " + getClass().getSimpleName() + ": will always use local fallback JMS");
        }
