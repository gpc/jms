...
class ServiceInspector {
    ...
    String resolveDestinationName(final String name, grailsApplication) {
        ...
        if (resolvedName) {
            final List<String> pathTokens = name.tokenize('.').reverse()
            def node = grailsApplication.config
            while( node && node instanceof Map && pathTokens.size() ) {
                node = node[pathTokens.pop()]
            }
            if ( node && pathTokens.size() == 0 ) {
                resolvedName = node
                LOG.info "destination with key '$name' resolved to '$resolvedName'." +
                        "The name '$resolvedName' will be used as destination."
            }
        }
        resolvedName
    }
